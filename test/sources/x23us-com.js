...
describe("顶点（x23us）数据源测试", function () {
  ...
  it("测试获取书籍信息", async function () {
    const book = await dingdian.detail('63f44e872b6faa293ccac88c40809543', new ResourceInformation(SourceLiteral.X23usCom, '503/'))
  ...
  it("测试获取章节内容", async function () {
    const chapter = await dingdian.chapter(
      ...
      new ResourceInformation(SourceLiteral.X23usCom, '503/49020.html')
    )
