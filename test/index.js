...
async function runTest() {
  try {
    testConfig = JSON.parse(
      fs
        .readFileSync(path.resolve(__dirname, "./test-config.json"))
        .toString("utf-8")
    );
  } catch {
  ...
  if (testConfig.baseUrl) {
    ...
    baseUrl =
      "http://" + testConfig.host + ":" + testConfig.port + testConfig.basePath;
  } else {
...
async function runTests(testcase, route = "") {
  ...
  if (testcase.type == "route") {
    ...
    for (let method in testcase) {
      ...
      //发起请求，并对结果进行正常性断言
      assertResult(
        await superAgent[method](route)
          .set("token", token)
          [sendMethod](testcase[method])
      );
    }
  } //路由级测试用例
  else if (testcase.type == "module" || !testcase.type) {
    for (const key in testcase) {
      await runTests(testcase[key], route + key);
    }
    return;
  } //模块级测试用例
}
