...
export class BookService {
  ...
  async getBook(source: string, bid: string) {
    ...
    if (!book) {
      book = await bookDao.getBook(bid);
    }
  ...
  async getCatalog(source: string, bid: string) {
    ...
    try {
      catalog = await provider.catalog(
        bid,
        book.catalogPageInfo ? book.catalogPageInfo : book.detailPageInfo
      );
      if (catalog.length) {
        const book = new Book();
      ...
      if (catalog) {
        catalog.forEach((i) => delete i["_id"]);
      }
      result.data = catalog;
    } catch (e) {
      handleError(e, provider.name);
  ...
  public async getBookList(
    ...
    try {
      ...
      for (const book of (bookList && bookList.books) || []) {
        bookDao.updateBook(book);
      }
    } catch (e) {
      handleError(e, provider.name);
