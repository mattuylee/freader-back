  ...
  async detail(bid: string, info: ResourceInformation) {
    ...
    try {
      ...
      const updateTimeText = $(
        "#maininfo #info p:nth-of-type(3)",
        mainInfo
      ).text();
      firstStop = updateTimeText.indexOf("：");
      lastStop = updateTimeText.indexOf(" ", firstStop);
      book.lastUpdateTime = updateTimeText
        .slice(firstStop + 1, lastStop)
        .trim();
      book.latestChapter = $("#maininfo #info p:nth-of-type(4) a", mainInfo)
        .text()
        .trim();
      book.words = null;
      book.cover = base + $("#sidebar #fmimg img", mainInfo).attr("src").trim();
      book.intro = $("#maininfo #intro p", mainInfo)
        .text()
        .trim()
        .split(/\s\s+/)
        .join("\n");
  ...
  async catalog(bid: string, info: ResourceInformation) {
    ...
    try {
      ...
      chpaterEls.forEach((chapterEl, index) => {
        ...
        chapter.resourceInfo.data = path.posix.join(
          info.data,
          $(chapterEl).attr("href").trim()
        );
        chapter.makeId();
