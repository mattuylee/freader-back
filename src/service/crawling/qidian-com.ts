...
const UA =
  ...
  rankSupport =
    SeriesSupport.MultiPage | SeriesSupport.Gender | SeriesSupport.Category,
  categorySupport =
    SeriesSupport.MultiPage | SeriesSupport.Gender | SeriesSupport.FinishState,
  recommendSerieses = [
    {
      id: "bestSelllist",
...
export class Qidian implements ResourceProvider {
  ...
  public async search(keyword: string, page?: number): Promise<Book[]> {
    ...
    const response = (await superAgent
      .get("https://www.qidian.com/search")
      .set("User-Agent", UA)
    ...
    for (const item of items.toArray()) {
      try {
        ...
        book.author = $(".book-mid-info .author img", item)
          .next()
          .text()
          .trim();
        const anchors = $(".book-mid-info .author a", item);
        if (anchors.length > 1) {
          book.category = $(anchors[anchors.length - 1])
            .text()
            .trim();
        }
        const staus = $(".book-mid-info .author span", item).text().trim();
        book.status =
          staus === "完结" ? UpdateStatus.Finished : UpdateStatus.Serial;
        book.intro = $(".book-mid-info .intro", item).text().trim();
        book.words = $(".book-right-info .total p:first-of-type span", item)
          .text()
          .trim();
        book.latestChapter = $(".book-mid-info .update a", item)
          .text()
          .slice(5)
          .trim();
        book.lastUpdateTime = $(".book-mid-info .update span", item)
          .text()
          .trim();
        ...
        book.detailPageInfo = new ResourceInformation(
          this.name,
          JSON.stringify({
            name: book.name,
            author: book.author,
  ...
  async catalog(bid: string, info: ResourceInformation): Promise<Chapter[]> {
    ...
    const res = await agent
      .get("https://book.qidian.com/ajax/book/category")
      .query({
        bookId: qdId,
        _csrfToken: token,
      })
  ...
  async chapter(
    ...
    try {
      const res = await superAgent
        .get("https://read.qidian.com/chapter/" + info.data)
        .timeout({
          deadline: 30000,
        });
      const $ = cheerio.load(res.text),
        chapter = new Chapter();
      chapter.title = $("#j_chapterBox .main-text-wrap .j_chapterName")
        .text()
        .trim();
      chapter.wordCount = parseInt(
        $("#j_chapterBox .main-text-wrap .j_chapterWordCut").text(),
        10
      );
      if ($("#j_chapterBox .main-text-wrap .vip-limit-wrap").length) {
        ...
        chapter.content = "";
        const paragraphs = $(
          "#j_chapterBox .main-text-wrap .read-content p"
        ).toArray();
  ...
  private async fetchBookList(
    ...
    if (series.type === SeriesType.Category) {
      ...
      series.support & SeriesSupport.Category &&
    ...
    for (const bookInfo of records) {
      ...
      book.status =
        bookInfo.state === "完结" ? UpdateStatus.Finished : UpdateStatus.Serial;
  ...
  private async getCsrfToken(): Promise<string> {
    if (Date.now() - this.csrfToken.fetchTime > 86400000 * 7) {
      ...
      const response = await superAgent.get(
          "https://www.qidian.com/ajax/Help/getCode"
        ),
        cookie = response
          .get("Set-Cookie")
          .find((cookie) => cookie.startsWith("_csrfToken="));
      this.csrfToken.fetchTime = Date.now();
      this.csrfToken.token = cookie.slice(
        cookie.indexOf("=") + 1,
        cookie.indexOf(";")
      );
    }
