...
class CronService {
  ...
  public enable() {
    ...
    logger.info(
      "计划任务已启动，下次执行时间：" +
        util.formatTime(new Date(t), "chineseFull")
    );
    setTimeout(() => {
      this._doCheck();
    }, t - Date.now());
  ...
  private _getNextCheckTime() {
    ...
    while (t < Date.now()) {
      t += globalConfig.cron.checkPeriod * 1000;
    }
    return t;
  }
  ...
  private async _checkBooks() {
    ...
    while (await books.hasNext()) {
      ...
      if (!provider) {
        logger.warn(
          `无效的数据源【${oldbook.source}】\nfrom 【${oldbook.name}】\nbid:${oldbook.bid}】`
        );
        continue;
      }
      try {
        let newbook = await provider.detail(
          oldbook.bid,
          oldbook.detailPageInfo
        );
        ...
        //完本或有新章节时更新目录
        if (
          newbook.status == UpdateStatus.Finished ||
          !filter.title ||
          !(await cronDao.getChapter(filter))
        ) {
          logger.debug(`更新书籍目录【${newbook.name}】`);
          let catalog = await provider.catalog(
            newbook.bid,
            newbook.catalogPageInfo
              ? newbook.catalogPageInfo
              : newbook.detailPageInfo
          );
    ...
    logger.debug(
      `扫描到${updateCount}条书籍记录需要更新，成功更新：${updatedCount}，产生异常：${exceptionCount}`
    );
    await books.close();
  }
}
