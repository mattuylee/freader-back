    ...
    for (let route of controller.routes) {
        for (let request of route.requests) {
            innerRouter[request.method](route.path, async (req, res) => {
                ...
                if (request.token) {
                    await userService.assertToken(<string>req.headers.token).catch(e => {
                        result = e
                    })
                    if (result.error) { res.json(result); return }
                } //身份认证
                let callback: Function, thisObject: object
                if (!request.invoke) { res.json(result); return }
                else if (typeof request.invoke == 'function') {
                    callback = request.invoke
                    thisObject = request.thisObject ? request.thisObject : controller.service
                }
