...
export class BookDao {
  ...
  async updateCatalog(bid: string, source: string, catalog: Chapter[]) {
    ...
    let existIds = (await chapterCollection.find({ bid: bid, source: source }, {
      sort: { index: 1 },
      projection: { cid: true }
    }).toArray()).map(c => c.cid)
    let nextIndex = 0
    for (; nextIndex < existIds.length; ++nextIndex) {
      if (!catalog[nextIndex] || !catalog[nextIndex].cid !== existIds[nextIndex]) {
        break;
      }
    }
    if (nextIndex < existIds.length) {
      //已存在的没有遍历完，说明存在失效章节，删除失效的章节
      await chapterCollection.deleteMany({ cid: { $in: existIds.slice(nextIndex) } })
    }
